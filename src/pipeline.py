...
class BioLitGraphPipeline:
    ...
    def run(self, query: str, retmax: int = 60, source: str = 'live') -> dict[str, Any]:
        ...
        if source == 'demo':
            ...
            if not ids:
                raise RuntimeError(
                    'PubMed returned no papers for that query. Try a broader topic or switch to demo mode.'
                )

            papers = self.pubmed.fetch_summaries(
                ids,
        ...
        def safe_display_path(path) -> str:
            try:
                rel = path.relative_to(settings.user_data_dir)
                return f"user_data/{str(rel).replace('\\\\', '/')}"
            except ValueError:
                try:
                    rel = path.relative_to(settings.base_dir)
                    return f"bundle/{str(rel).replace('\\\\', '/')}"
                except ValueError:
                    return str(path)
        ...
        bundle = {
            ...
            'outputs': {
                'papers_csv': safe_display_path(papers_path),
                'entities_csv': safe_display_path(entities_path),
                'relations_csv': safe_display_path(relations_path),
                'graph_json': safe_display_path(graph_path),
            },
        }

        write_json(run_dir / 'bundle.json', bundle)
        return bundle                raise RuntimeError('PubMed returned no papers for that query. Try a broader topic or switch to demo mode.')
